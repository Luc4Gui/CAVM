...
# ssim = StructuralSimilarityIndexMeasure(return_full_image=True).cuda()
ssim = StructuralSimilarityIndexMeasure(sigma=7,kernel_size=11,return_full_image=True).cuda()
...
def __percentile_clip(input_tensor, reference_tensor=None, p_min=0.5, p_max=99.5, strictlyPositive=True):
    ...
    v_min, v_max = np.percentile(reference_tensor.cpu(), [p_min,p_max]) #get p_min percentile and p_max percentile

    if( v_min < 0 and strictlyPositive): #set lower bound to be 0 if it would be below
        v_min = 0
    output_tensor = torch.clip(input_tensor,v_min,v_max) #clip values to percentiles from reference_tensor
...
def compute_metrics_mask(gt_image: torch.Tensor, prediction: torch.Tensor, mask: torch.Tensor, normalize=True):
    ...
    if normalize:
        reference_tensor = gt_image #use all the tissue that is not masked for normalization
        gt_image_infill = __percentile_clip(gt_image_infill, reference_tensor=reference_tensor, p_min=0, p_max=100, strictlyPositive=True)
        prediction_infill = __percentile_clip(prediction_infill, reference_tensor=reference_tensor, p_min=0, p_max=100, strictlyPositive=True)
...
def compute_metrics(gt_image: torch.Tensor, prediction: torch.Tensor, normalize=True):
    """Computes MSE, PSNR and SSIM between two images only in the masked region.
    ...
    # Normalize to [0;1] based on GT (otherwise MSE will depend on the image intensity range)
    if normalize:
        reference_tensor = gt_image
        gt_image = __percentile_clip(gt_image, reference_tensor=reference_tensor, p_min=0.5, p_max=99.5, strictlyPositive=True)
        prediction = __percentile_clip(prediction, reference_tensor=reference_tensor, p_min=0.5, p_max=99.5, strictlyPositive=True)


    # SSIM - apply on complete masked image but only take values from masked region
    full_cuboid_SSIM, ssim_idx_full_image = ssim(preds=prediction, target=gt_image)
    ...
    # MSE
    MSE = mse(preds=prediction, target=gt_image)

    # PSNR
    PSNR = psnr(preds=prediction, target=gt_image)
